...
export function MatchFlow({
  ...
  const myReadyAt = isCreator ? room.creator_ready_at : room.opponent_ready_at;
  const opponentReadyAt = isCreator
    ? room.opponent_ready_at
    : room.creator_ready_at;
  const opponentName = isCreator
    ? room.opponent?.profile_name ?? "your opponent"
    : room.creator?.profile_name ?? "the host";
  ...
  return (
    <div className="rounded-xl border border-base-border bg-base-surface p-5">
      ...
      {!room.room_code ? (
          ...
          <form onSubmit={handleSetCode} className="mt-3 space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="btn-3d btn-3d-volt rounded-lg px-4 py-2 font-semibold disabled:opacity-50"
              >
                Share ID
              </button>
            </div>
        ...
        <div className="mt-3 flex items-center justify-between rounded-lg bg-base-raised px-4 py-3">
          ...
          {!isCreator && (
            <button
              onClick={handleCopy}
              className="btn-3d btn-3d-outline rounded-lg px-3 py-1.5 text-sm font-semibold"
            >
              {copied ? "Copied!" : "Copy"}
            </button>
          )}
      ...
      {/* Step 2: start / finish — each player has to click Start themselves */}
      {room.room_code && (
        <div className="mt-4">
          {!room.started_at ? (
            <div className="flex flex-wrap items-center gap-3">
              {opponentReadyAt && !myReadyAt && (
                <p className="w-full rounded-lg border border-volt/30 bg-volt/10 px-3.5 py-2 text-sm font-semibold text-volt">
                  {opponentName} has hit Start match and is waiting for you.
                </p>
              )}
              {myReadyAt ? (
                <span className="inline-flex items-center gap-2 text-sm font-semibold text-warn">
                  <span className="live-dot h-2 w-2 rounded-full bg-warn" />
                  Waiting for {opponentName} to start the match…
                </span>
              ) : (
                <button
                  onClick={handleStart}
                ...
                >
                  {busy
                    ? "Starting..."
                    : opponentReadyAt
                    ? "Opponent is waiting — Start match"
                    : "Start match"}
                </button>
              )}
            </div>
